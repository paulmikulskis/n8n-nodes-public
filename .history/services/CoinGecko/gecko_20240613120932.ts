...
import type { CoinGecko } from "@ape-analytics/validators";
...
export class CoinGeckoClient {
  ...
  async getSimpleTokenPrice(
    ...
    return this.fetchData<CoinGecko.custom.SimplePriceResponse>(
      "/simple/price",
      params,
    );
  }
  ...
  async getHistoricalChartApiData(
    ...
    try {
      const response =
        await this.fetchData<CoinGecko.api.HistoricalChartDataResponse>(
  ...
  async getSimpleHistoricalTokenPrice(
    ...
    try {
      ...
      if (response.ok) {
        ...
        return new Err(
          new Error(`Failed to fetch data: ${(response.val as Error).message}`),
        );
      }
    } catch (error) {
      return new Err(error as Error);
  ...
  async getAllCoins(params?: {
    ...
    const response = await this.fetchData<CoinGecko.api.CoinsListResponse[]>(
      "/coins/list",
      params,
    );
    if (response.ok) {
      this.supportedCoins = response.val;
      this.supportedCoinsMap = response.val.reduce(
        (acc, coin) => {
  ...
  async getCoinData(
    ...
    return this.fetchData<CoinGecko.api.CoinDataResponse>(
      `/coins/${coinID}`,
      params,
    );
  }
  ...
  async getEZTokenPrice(
    ...
    if (!geckoSupportedVSCurrencies.includes(vs)) {
      ...
      if (response.ok && !response.val.includes(vs)) {
        return new Err(
          new Error(`Token versus symbol '${vs}' is not supported`),
        );
      }
    }
    ...
    if (!coinID) {
      return new Err(
        new Error(
          `Token price requested symbol '${tokenISOCode}' is not whitelisted`,
        ),
      );
    }
    ...
    if (priceResult.ok) {
      ...
      if (!data) {
        return new Err(
          new Error(`Failed to fetch the price for '${tokenISOCode}'`),
        );
      }
      const res = data[vs];
      if (!res) {
