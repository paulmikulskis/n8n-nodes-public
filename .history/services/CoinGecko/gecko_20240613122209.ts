...
export class CoinGeckoClient {
	...
	public supportedCoins: types.api.CoinsListResponse[] = [];
	public cacheControl: {
	...
	async getSimpleTokenPrice(
		params: types.custom.SimplePriceParams,
	): Promise<Result<types.custom.SimplePriceResponse, Error>> {
		return this.fetchData<types.custom.SimplePriceResponse>('/simple/price', params);
	}

	async getHistoricalChartApiData(
		params: types.api.HistoricalChartDataParams,
	): Promise<Result<types.api.HistoricalChartDataResponse, Error>> {
		const {
			id: coinId,
			from: fromDateInt,
			to: toDateInt,
		...
		try {
			const response = await this.fetchData<types.api.HistoricalChartDataResponse>(
	...
	async getSimpleHistoricalTokenPrice(
		args: types.custom.SimpleDailyHistoricPriceParams,
	): Promise<Result<types.custom.SimpleDailyHistoricPriceResponse, Error>> {
	...
	}): Promise<Result<types.api.CoinsListResponse[], Error>> {
		if (this.cacheControl.getAllCoins.called) {
			return Ok(this.supportedCoins);
		}

		const response = await this.fetchData<types.api.CoinsListResponse[]>('/coins/list', params);
		if (response.ok) {
	...
	): Promise<Result<types.api.CoinDataResponse, Error>> {
		return this.fetchData<types.api.CoinDataResponse>(`/coins/${coinID}`, params);
	}
